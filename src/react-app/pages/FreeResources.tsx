  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-6xl mx-auto px-6 py-20">
        <h1 className="text-5xl font-bold text-center text-gray-900 mb-4">Free Resources</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {resources.map((resource, index) => (
            <div key={index} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition">
              <span className="inline-block px-3 py-1 bg-blue-100 text-blue-600 text-sm font-semibold rounded mb-3">
                {resource.type}
              </span>
              ...
              {resource.download ? (
                ...
                <a href={`/blog/${resource.title.toLowerCase().replace(/\s+/g, '-')}`} className="text-blue-600 font-semibold hover:text-blue-700">
                  Read Article →
                </a>
              )}
            </div>
        ...
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-6">Blog Articles</h2>
          <div className="space-y-4">
            {resources.filter(r => !r.download).map((article, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-6 hover:bg-gray-100 transition">
                ...
                <a href={`/blog/${article.title.toLowerCase().replace(/\s+/g, '-')}`} className="text-blue-600 font-semibold hover:text-blue-700">
                  Read More →
                </a>
              </div>
            ))}
